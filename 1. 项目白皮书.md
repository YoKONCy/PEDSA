...
| `src/data/` | 数据与存储 | `storage.rs` (mmap/SoA/SIMD), `store.rs` (PedsaStore trait + 同步工具), `sqlite_store.rs` (SQLite 后端), `dataset.rs`, `data_loader.rs` |
| `src/bench/` | 基准测试 | `benchmarks.rs` (V3 + 千万级), `benchmark_latency.rs` (向量化延迟) |
...
*   **存储解耦**：`PedsaStore` trait 将数据持久化从 `AdvancedEngine` 中剥离。引擎保持纯内存 `AHashMap` 架构不变，持久化由外部 `SqliteStore` 等后端通过 `export_engine_to_store` / `import_store_to_engine` 双向同步。
...
| **持久化** | `export_to_sqlite(path)` / `import_from_sqlite(path)` | SQLite 双向同步 |
| | `SqliteStore(path)` | 独立 SQLite 存储操作 |
