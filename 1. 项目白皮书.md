...
## 1. 千万级压力测试表现 (Extreme Stress Test - V2)

在 V2 架构实现“索引-载体分离 (Index-Carrier Split)”后，我们针对 **1000 万级** 节点的实时检索场景进行了全新评估。

| 指标 | V2 (Chaos Optimized) | 备注 |
| **总节点数** | 10,000,000+ | 包含磁盘数据与热插入缓冲区 |
| **内存占用** | **~380 MB** | 增加 Chaos SoA 存储空间 |
| **单次全量扫描耗时** | **579 μs ** | 极限性能 |
| **混沌检索全量扫** | **132 ms (Two-stage)** | 极致优化 |
| **热插入支持** | **支持** | 无需重启即可实现增量更新 |

---

## 2. V2 架构演进：全量拥抱 TriviumDB (Index-Carrier Split)

PEDSA V2 是一次彻底的底层重构，从传统的“内存对象树”与手工 SQLite 持久化，转向了全面接入专注 AI 原生的统一存储基座——**[TriviumDB](https://crates.io/crates/triviumdb)**。
所有底层的内存切割、零拷贝与算子加速皆由 `TriviumDB` 一力承担，PEDSA 彻底蜕变为一个纯粹的**上层认知与图谱扩散调度引擎** (Cognitive Pipeline)。

### A. 索引-载体分离 (Index-Carrier Split - Powered by TriviumDB)
*   **Hot Index (热索引)**：TriviumDB 将所有节点的 SimHash 指纹、ID 和元数据提取出来，以 **Struct of Arrays (SoA)** 布局存储。
*   **Chaos Store (混沌引擎)**：在 TriviumDB 的强力支持下，混沌指纹 (512-bit) 与混沌向量也直接内生重构为了底层 SoA 布局。通过将特征和向量数据在内存中连续存放，显著提升了线性扫描时的缓存局部性。
...
*   **空间降维**：V2 彻底移除了 SimHash 中难以维护的空间 (Location) 哈希维度。地理位置信息现在完全通过 **Ontology (本体层)** 的双向语义匹配实现（如：识别“深圳”直接激活关联事件），这比不精确的空间哈希更加可靠。
...
针对千万级规模下的“混沌共振”，V2 引入了高效的两段式过滤机制：
...
## 3. 检索流转过程 (V2 Dual-Track Flow)

当用户输入文本时，V2 检索流转升级为**理性 (Rational) 与 混沌 (Chaotic) 双轨并行架构**：
...
## 6. 源码架构 (Source Architecture)

项目采用按职责域分层的模块化架构，`main.rs` 仅保留 20 行 CLI 路由逻辑：

| 子目录 | 职责 | 核心文件 |
| :--- | :--- | :--- |
| `src/core/` | 引擎核心 | `engine.rs` (AdvancedEngine), `retrieval.rs` (检索管线 + DPP), `ontology.rs` (本体维护), `simhash.rs` (指纹), `types.rs` (数据结构) |
| `src/ml/` | 模型集成 | `embedding.rs` (Candle BGE-M3), `gliner_ner.rs` (GLiNER NER, 可选), `inference_engine.rs` (GGUF) |
| `src/data/` | 数据与存储 | 全面依托 TriviumDB 引擎完成零拷贝序列化与高性能持久化, `dataset.rs`, `data_loader.rs` |
| `src/bench/` | 基准测试 | `benchmarks.rs` (V2 + 千万级), `benchmark_latency.rs` (向量化延迟) |
| `src/` | 入口与绑定 | `main.rs` (CLI), `lib.rs` (库导出), `tests.rs` (集成测试), `python.rs` (PyO3 绑定) |

**设计原则**：
*   **跨文件 `impl`**：`AdvancedEngine` 定义在 `engine.rs`，检索管线 (`retrieval.rs`) 和本体维护 (`ontology.rs`) 通过跨文件 `impl` 块扩展其功能。
*   **职责解耦**：`main.rs` 不包含任何业务逻辑，`tests.rs` 完全自包含引擎初始化流程。
*   **存储基座升级**：引擎现已全面接入 TriviumDB，彻底摒弃了传统的 SQLite 关系型存储和双向同步负担。所有的数据持久化、零拷贝 Mmap、双路混合检索与 FISTA 等增强算子均由 TriviumDB 底层强力驱动。
*   **Feature 隔离**：`gliner` (ONNX NER) 和 `python` (PyO3) 两个 feature flag 分别控制对应编译链路，互不干扰。

---

## 7. Python 接入 (PyO3 Bindings)

通过 `maturin develop --release` 构建后，Python 侧可 `import pedsa` 使用完整的图谱引擎。

### Python API 一览

| 分类 | 方法 | 说明 |
| :--- | :--- | :--- |
| **构建** | `get_or_create_feature(word) → int` | 自动分配 Feature ID（停用词返回 -1） |
| | `add_feature(id, keyword)` | 手动指定 ID 添加特征节点 |
| | `add_event(id, summary)` | 添加事件节点（自动提取指纹/情感/时间） |
| | `add_edge(src, tgt, weight)` | 建立记忆图关联 |
| | `maintain_ontology(source, target, type, strength)` | 维护本体关联 |
| | `execute_maintenance(action, ...)` | 统一维护接口 (upsert/replace) |
| **编译** | `compile()` | 构建 AC 自动机、入度索引、时序脊梁 |
| **检索** | `retrieve(query, ref_time=0, chaos_level=0.0)` | 多级检索，返回 `[(id, score)]` |
| **查询** | `get_node(id) → dict` | 节点完整信息 |
| | `get_edges(id)` / `get_ontology_edges(id)` | 图邻居遍历 |
| | `all_node_ids()` / `all_feature_keywords()` | 枚举全部节点/关键词 |
| | `keyword_to_id(keyword) → int?` | 关键词查找节点 ID |
| **维护** | `apply_decay(rate, threshold)` | 全局衰减与剪枝 |
| | `trigger_arbitration(source)` / `apply_arbitration(...)` | 逻辑仲裁 |
| **持久化** | 委托给 `TriviumDB` 底层引擎自动管理 | 原生 WAL 与 Flush 机制，替代冗余的双向同步 |

---

## 8. 测试场景覆盖 (Tests Summary)

目前已封装 11 个核心测试场景，确保逻辑稳健：
1.  **语义对齐**：佩罗 -> Pero。
